...
<div id="yith-wapo-block-<?php echo esc_attr( $block->id ); ?>" class="<?php echo esc_attr( $block_classes ); ?>">
	...
	foreach ( $addons as $key => $addon ) :
		...
		if ( yith_wapo_is_addon_type_available( $addon->type ) ) :
			...
			$current_location = isset($_POST['location_price']) ? $_POST['location_price'] : 
	                      (isset($_COOKIE['location_price']) ? $_COOKIE['location_price'] : '');
			$nbt_locations = $addon->get_setting( 'nbt_locations' );
			if($nbt_locations == '' || $nbt_locations == $current_location || $nbt_locations == 'all' ){
